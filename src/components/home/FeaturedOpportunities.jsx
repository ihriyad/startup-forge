  ...
  return (
    <section className="max-w-280 mx-auto px-6 py-16 border-t border-default-100">
      ...
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        ...
        <Link
          href="/opportunities"
          className="flex items-center gap-1.5 text-xs uppercase  font-bold tracking-wider hover:text-violet-700 transition-colors"
        >
        ...
        {opportunities.map((opp, i) => {
          ...
          return (
            <ScrollReveal key={opp._id} delay={i * 80}>
              ...
              >
                <Card className="h-full bg-content1 border-2 hover:bg-background  border-transparent transition-all duration-300 rounded-xl shadow-sm min-h-[300px] flex flex-col justify-between">
                  {/* Upper Structural Info Segment */}
                  <div className="flex flex-col flex-1 w-full">
                    <Card.Header className="flex items-start justify-between gap-4 p-6 pb-2">
                      <div className="flex flex-col gap-1">
                        <Card.Title className="text-base sm:text-lg md:text-xl text-foreground tracking-normal leading-snug">
                          {opp.role_title}
                        </Card.Title>
                        <div>
                         <span className="text-sm  font-medium flex items-center gap-1 text-foreground">
                            
                            {opp.startup_name} <MdVerified />
                            </span>
                        </div>
                      </div>
                      <span
                    ...
                    <Card.Content className="px-6 py-3 flex flex-col gap-5">
                        <p className="text-sm text-foreground">Required Skills:</p>
                      <div className="flex flex-wrap gap-2">
                        {opp.required_skills?.slice(0, 4).map((skill) => (
                          <span
                            key={skill}
                            className="text-[11px] px-3 py-1 rounded-md bg-violet-600/10 dark:bg-violet-600/20 text-violet-600 dark:text-violet-400 font-medium tracking-wide"
                          >
                            {skill}
                          </span>
                        ))}
